...
%MOVTRACK M-file for movtrack.fig
% visualization of .stk and .tif files overlaid with trajectories
% goodies: movies .avi, quantification of clusters...
%
...
function firstradiobutton_Callback(hObject, eventdata, handles)
...
function nromolDinst_Callback(hObject, eventdata, handles)
...
function ident_Callback(hObject, eventdata, handles)
    ident=get(hObject,'value');
    if ident==1
        set(handles.Dradiobutton,'value',0);
    end
    guidata(hObject, handles);
...
function loc_Callback(hObject, eventdata, handles)
    set(handles.blinking,'value',0);
    set(handles.rainbowradiobutton,'value',0);
    set(handles.timecolorradiobutton,'value',0);
    guidata(hObject, handles);

%function spineradiobutton_Callback(hObject, eventdata, handles)
%    set(handles.blinking,'value',0);
%    set(handles.rainbowradiobutton,'value',0);
%    set(handles.timecolorradiobutton,'value',0);
%    guidata(hObject, handles);
...
function trajradiobutton_Callback(hObject, eventdata, handles)
    handles.trajbutton=get(hObject,'value');
    handles.perival=1;
    set(handles.trcradiobutton,'value',0);
    set(handles.pkradiobutton,'value',0);
    set(handles.accel,'value',0);
...
def = {handles.color.all ,handles.color.extra,handles.color.peri,...
    handles.color.syn,handles.color.blink}; % default values
...
%   handles.param.Xdim=p(1);
%   handles.param.Ydim=p(2)/p(4);
%   handles.param.nfram=p(4);
%   set(handles.file1,'userdata',datamatrix); %images
%end
...
set(handles.zoomDinstpushbutton,'enable','on');  
...
if isempty(trcfile)==1
    ...
    set(handles.zoomDinstpushbutton,'enable','off');  
    ...
    if tipotrc<2
        if tipotrc==1
                x=load(trcfile);   %.trc
        elseif tipotrc==0
                [x,a,Te,nb_frames]=trajTRC(trcfile);
        end
        nrotraj=max(x(:,2));
    else  %tipotrc==2
       pk=load(trcfile);   %.pk
        ...
        for i=1:max(actualtrc(:,1))
            ...
            if isempty(indexmol)==0 && condition==1 % 
                ...
                if firstbutton==1 % primer punto
                    ...
                    if isempty(indextime)==0  
                         if size(auxtrc,2)>5
                            ...
                            if size(auxtrc,2)>6 && auxtrc(indextime(1),7)==1 
                    ...
                    clear auxtrc
                else % not firstbutton
                    if localiz==0 && timecolor==0 
                        if actualtrc(indexmol(size(indexmol,1)),2)<handles.param.actual  && blink==1 % blink period
               ...
               if diff==1 %D
                   ...
                   cifras=num2str(actualtrc(pos,1)) ;
                   space=size(cifras,2);
                   indexd=find(diffdata(:,1)==i);
                   if isempty(indexd)==0
                       D=diffdata(indexd,2);
                       text(actualtrc(pos,3)+1,actualtrc(pos,4)+1,sprintf('%0.0f',actualtrc(pos,1)),'Color',[1 1 0]);
                       text(actualtrc(pos,3)+(space*5),actualtrc(pos,4)+1,sprintf('%0.0000g',D),'Color',[1 1 1],'FontSize',7);
                   end
               end
...
mensaje = msgbox('Preparing visualization','Please wait');
...
if pkchoice==0
    ...
    if isempty(x2)==0 % second traj
       codecol='r';
       plottracesframe(x2,handles.param.actual,codecol,localiz,linew,handles)
    end
