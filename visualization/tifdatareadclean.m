function [stack_info,stack] = tifdatareadclean(filename)
% function [stack_info,stack] = tifdatareadclean(filename)
% stack: image/s
...
while (ifd_pos ~= 0)
    ...
    fseek(TIF.file, ifd_pos, -1);    %disp(strcat('reading img at pos :',num2str(ifd_pos)));
    %read in the number of IFD entries
    num_entries = fread(TIF.file,1,'uint16', TIF.BOS);    %disp(strcat('num_entries =', num2str(num_entries)));
    ...
    for i = 1:num_entries
        ...
        entry = readIFDentry;        %disp(strcat('reading entry <',num2str(TIF.entry_tag),'>'));
        ...
        switch TIF.entry_tag
            ...
            case 258         % BitsPerSample per sample
                ...
                IMG.bits           = TIF.BitsPerSample(1);      %fprintf(1,'BitsPerSample %i %i %i\n', entry.val);
            ...
            case 273         % strip offset
                ...
                TIF.StripNumber    = entry.cnt;                %fprintf(1,'StripNumber = %i, size(StripOffsets) = %i %i\n', TIF.StripNumber, size(TIF.StripOffsets));
            case 277         % sample_per pixel
                TIF.SamplesPerPixel  = entry.val;                %fprintf(1,'Color image: sample_per_pixel=%i\n',  TIF.SamplesPerPixel);
