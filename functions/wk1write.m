...
%   $Revision: 5.16.4.2 $  $Date: 2006/10/02 16:32:37 $
%
...
%
if ~ischar(filename),
    error('MATLAB:wk1write:filenameNotString', 'FILENAME must be a string.');
end

if nargin < 2
    error('MATLAB:wk1write:notEnoughInputs', 'Requires at least 2 input arguments.');
end
...
[fid, errmsg] = fopen(filename,'Wb', 'l');

if fid == -1
    error('MATLAB:wk1write:couldNotOpenFile', 'Could not open file "%s": %s.', ...
          filename, xlate(errmsg));
end
%
